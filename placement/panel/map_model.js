...
`, 50);
  ...
  function showProModeSignalIntro() {
    ...
    signalModal.className = 'tutorial-modal pro-signal-modal show';
  ...
  function showCompletionModal() {
    ...
    saveCompletionRecord({
      allowLeaderboard: !isPresetSolutionLoaded && !isLeaderboardLockedForCurrentRun
    });
  }
  ...
  function saveCompletionRecord(options = {}) {
    const allowLeaderboard = options.allowLeaderboard ?? (!isPresetSolutionLoaded && !isLeaderboardLockedForCurrentRun);
    if (!allowLeaderboard) {
      return;
    }
    ...
    if (!window.MeowtopiaAccount) {
      setTimeout(() => saveCompletionRecord({ allowLeaderboard }), 500);
      return;
    }
  ...
  function updateCatPlacementHints(grid) {
    ...
    cells.forEach(cell => cell.classList.remove('cat-placeable-cell'));
    ...
    cells.forEach((cell, index) => {
      ...
      cell.classList.toggle('cat-placeable-cell', isPlaceableNow);
    });
  }
  ...
  modeToggleBtn?.addEventListener('click', function () {
    ...
    shouldAnimateFishOnNextUpdate = false;
    hasShownCompletionModal = false;
    const shouldShowSignalIntro = isProModeGuidePending && !isGameMode && !hasCompletedProModeIntro;
