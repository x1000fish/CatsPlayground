...
function saveRecord_(payload, game) {
  ...
  const hasRankableTime = payload.timeRankable !== false && payload.time !== '' && payload.time !== null && typeof payload.time !== 'undefined';
  const time = hasRankableTime ? Number(payload.time || 0) : '';
...
function getLeaderboard_(game, level) {
  ...
  for (let row = 1; row < data.length; row++) {
    ...
    if (!existing ||
        ...
        (steps === existing.steps && rankNumber_(time) < rankNumber_(existing.time))) {
      const profile = profiles[userKey] || {};
      bestByUser[userKey] = {
        username: profile.username || username,
        level:    level,
        steps:    steps,
        time:     rankNumber_(time) === Infinity ? '' : Number(time),
        photo:    profile.photo  || '',
